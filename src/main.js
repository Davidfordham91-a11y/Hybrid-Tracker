import { APP_VERSION, BUILD_NUMBER, IDB_NAME, IDB_STORE, SCHEMA_VERSION, STORAGE_KEY } from "./core/version.js";
import { setLooksUnusual } from "./utils/validation.js";
...
(() => {
  ...
  const DEFAULT_PROFILE = {
    ...
    units: "metric",
    gymLocation: "",
    preferredRunTime: "05:30",
    preferredGymTime: "12:00-13:00",
    primaryRace: {
      name: "Marathon",
      date: "2026-10-04",
      distanceKm: 42.2
  ...
  const LEGACY_PROFILE_DEFAULTS = {
    ...
    timezone: "Australia/Brisbane",
    units: "metric",
    gymLocation: "Jetts Albany Creek",
    preferredRunTime: "05:30",
    preferredGymTime: "12:00-13:00",
    primaryRace: {
      name: "Marathon",
  ...
  function loadState() {
    ...
    if ((saved.schemaVersion || saved.version || 0) < SCHEMA_VERSION && Object.keys(saved).length) {
      try {
        localStorage.setItem(`${STORAGE_KEY}:pre-v5-backup:${todayISO()}`, JSON.stringify(saved));
      } catch (_) {}
    }
  ...
  function migrateState(saved) {
    ...
    const existingVersion = Number(saved.schemaVersion || saved.version || 0);
    const hasExistingData = Object.keys(saved || {}).length > 0;
    const profile = migrateProfile(saved.profile || {}, existingVersion, hasExistingData);
    const starterMetric = {
      id: id("metric"),
      date: todayISO(),
    ...
    return {
      ...
      training: migrateTrainingState(saved.training || {}, now, deviceId),
      auth: {
      ...
      friends: {
        ...
        requests: saved.friends?.requests || [],
        lastFetchedAt: saved.friends?.lastFetchedAt || "",
        pendingLastFetchedAt: saved.friends?.pendingLastFetchedAt || "",
      ...
      settings: {
        ...
        units: settings.units || profile.units || "metric",
        timezone: settings.timezone || profile.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "Australia/Brisbane",
        updateLaterUntil: settings.updateLaterUntil || ""
      }
    };
  }

  function migrateProfile(source = {}, version = 0, hasExistingData = false) {
    const base = hasExistingData && version < 5 ? LEGACY_PROFILE_DEFAULTS : DEFAULT_PROFILE;
    const raceSource = source.primaryRace || {};
    const raceDate = source.raceDate || raceSource.date || base.primaryRace.date;
    return {
      displayName: cleanProfileText(source.displayName || source.name || base.displayName, 60),
      bodyweightKg: optionalNumber(source.bodyweightKg ?? source.bodyweight ?? base.bodyweightKg),
      heightCm: optionalNumber(source.heightCm ?? source.height ?? base.heightCm),
      timezone: cleanProfileText(source.timezone || base.timezone, 80),
      units: source.units === "imperial" ? "imperial" : "metric",
      gymLocation: cleanProfileText(source.gymLocation || source.location || base.gymLocation, 100),
      preferredRunTime: normalizeProfileTime(source.preferredRunTime || source.runTime || base.preferredRunTime),
      preferredGymTime: cleanProfileText(source.preferredGymTime || source.gymTime || base.preferredGymTime, 40),
      primaryRace: {
        name: cleanProfileText(raceSource.name || source.raceName || base.primaryRace.name, 80),
        date: isISODate(raceDate) ? raceDate : base.primaryRace.date,
        distanceKm: optionalNumber(raceSource.distanceKm ?? source.raceDistanceKm ?? base.primaryRace.distanceKm) || 42.2
      }
    };
  }

  function cleanProfileText(value, limit = 80) {
  ...
  function normalizeProfileTime(value) {
    const text = String(value || "").trim();
    return /^\d{2}:\d{2}$/.test(text) ? text : DEFAULT_PROFILE.preferredRunTime;
  }
  ...
  function normalizeNutritionEntries(entries, now, deviceId) {
    ...
    Object.entries(entries || {}).forEach(([date, list]) => {
      normalized[date] = (list || []).map((entry) => normalizeFoodRecord({
  ...
  function cleanFoodPresets(presets) {
    ...
    presets.forEach((preset) => {
      ...
      if (!byName.has(key)) byName.set(key, normalizeFoodRecord({ ...preset, id: preset.id || id("preset"), name }, preset.estimated ? "restaurant-estimate" : "package-label"));
    });
    DEFAULT_PRESETS.forEach((preset) => {
      const key = normalizeName(preset.name);
      if (!byName.has(key)) byName.set(key, normalizeFoodRecord(preset, preset.estimated ? "restaurant-estimate" : "package-label"));
    });
    return [...byName.values()];
  }

  function normalizeFoodRecord(item = {}, fallbackSourceType = "user-entered") {
    const sourceType = normalizeFoodSourceType(item.sourceType || mapLegacyFoodSource(item.source) || fallbackSourceType);
    const confidence = normalizeFoodConfidence(item.confidence || (item.estimated ? "medium" : sourceType === "package-label" ? "verified" : "medium"));
    const estimated = Boolean(item.estimated) || ["restaurant-estimate", "ai-estimate"].includes(sourceType) || ["medium", "low"].includes(confidence);
    return {
      ...item,
      id: item.id || id("food"),
      name: String(item.name || "").trim(),
      brand: item.brand || "",
      servingDescription: item.servingDescription || item.serving || item.portion || "1 serving",
      servingWeightGrams: optionalNumber(item.servingWeightGrams || item.servingGrams || ""),
      calories: toNumber(item.calories),
      protein: toNumber(item.protein),
      carbohydrates: toNumber(item.carbohydrates ?? item.carbs),
      carbs: toNumber(item.carbs ?? item.carbohydrates),
      fat: toNumber(item.fat),
      fibre: toNumber(item.fibre),
      sodium: toNumber(item.sodium),
      caffeine: toNumber(item.caffeine),
      sourceType,
      sourceReference: item.sourceReference || item.source || "",
      confidence,
      verifiedAt: item.verifiedAt || (confidence === "verified" ? item.updatedAt || item.createdAt || "" : ""),
      isFavourite: Boolean(item.isFavourite),
      isPreset: item.isPreset !== undefined ? Boolean(item.isPreset) : Boolean(item.id && String(item.id).startsWith("preset")),
      estimated
    };
  }

  function mapLegacyFoodSource(source) {
    const value = normalizeName(source);
    if (value === "barcode") return "barcode";
    if (value === "meal-prep" || value === "recipe") return "recipe";
    if (value === "photo") return "ai-estimate";
    if (value === "preset") return "";
    if (value === "manual") return "user-entered";
    return "";
  }

  function normalizeFoodSourceType(value) {
    const allowed = ["package-label", "official-source", "barcode", "recipe", "restaurant-estimate", "user-entered", "ai-estimate"];
    return allowed.includes(value) ? value : "user-entered";
  }

  function normalizeFoodConfidence(value) {
    const allowed = ["verified", "high", "medium", "low"];
  ...
  function migrateTrainingState(training = {}, now, deviceId) {
    const templates = seedTrainingTemplates();
    return {
      templates: { ...templates, ...(training.templates || {}) },
      assignedPrograms: {
        running: {
          id: training.assignedPrograms?.running?.id || "assigned-runa-marathon-2026",
          templateId: training.assignedPrograms?.running?.templateId || "runna-marathon-2026-v1",
          userId: training.assignedPrograms?.running?.userId || "local",
          startDate: training.assignedPrograms?.running?.startDate || PLAN_WEEKS[0].start,
          status: training.assignedPrograms?.running?.status || "active",
          customisations: training.assignedPrograms?.running?.customisations || {},
          ...recordMeta(training.assignedPrograms?.running || {}, "assigned-runa-marathon-2026", now, deviceId)
        },
        gym: {
          id: training.assignedPrograms?.gym?.id || "assigned-hybrid-gym-split",
          templateId: training.assignedPrograms?.gym?.templateId || "hybrid-gym-split-v1",
          userId: training.assignedPrograms?.gym?.userId || "local",
          startDate: training.assignedPrograms?.gym?.startDate || PLAN_WEEKS[0].start,
          status: training.assignedPrograms?.gym?.status || "active",
          customisations: training.assignedPrograms?.gym?.customisations || {},
          ...recordMeta(training.assignedPrograms?.gym || {}, "assigned-hybrid-gym-split", now, deviceId)
        }
      },
      scheduledSessionEdits: training.scheduledSessionEdits || {}
    };
  }

  function seedTrainingTemplates() {
    return {
      "runna-marathon-2026-v1": {
        id: "runna-marathon-2026-v1",
        name: "Runna Marathon Block",
        type: "running",
        description: "Default 16-week marathon block ending with a 42.2 km race.",
        version: 1,
        sessions: RUN_PLAN
      },
      "hybrid-gym-split-v1": {
        id: "hybrid-gym-split-v1",
        name: "Hybrid Gym Split",
        type: "gym",
        description: "Monday to Friday hybrid athlete split with running-aware leg volume.",
        version: 1,
        sessions: Object.entries(GYM_TEMPLATES).map(([day, template]) => ({ day, ...template }))
      }
    };
  }
  ...
  function handleClick(event) {
    ...
    if (action === "accept-friend" || action === "reject-friend" || action === "remove-friend" || action === "block-friend") {
      updateFriendRelationship(actionEl.dataset.friendCode, action.replace("-friend", ""));
      return;
    }
    if (action === "sign-out") {
      signOut();
    ...
    if (action === "export-backup-first") {
      exportData();
      state.cloud.migrationBackupKey = `${STORAGE_KEY}:pre-v5-backup:${todayISO()}`;
      saveState({ skipSync: true });
      render();
      return;
    }
    ...
    if (action === "complete-set") {
      if (completeSet(date, exerciseId, setId)) {
        startRestTimer(60);
        saveState();
        render();
      }
    ...
    if (action === "complete-next-set") {
      if (completeNextSet(date, exerciseId)) {
        startRestTimer(60);
        saveState();
        render();
      }
    ...
    if (action === "save-training-programs") {
  ...
  function renderToday() {
    ...
    const raceDate = profile.primaryRace.date;
        ...
        <article class="hero-card">
          <div class="hero-top">
            <div>
              ...
              <h2>Dashboard</h2>
              <p class="subtle">${escapeHtml(profile.preferredRunTime)} run - ${escapeHtml(profile.preferredGymTime)} lift - ${escapeHtml(profile.primaryRace.name)} ${formatShortDate(raceDate)}</p>
            </div>
            <span class="badge race">${daysUntil(raceDate)} days</span>
          </div>
          <div class="ring-grid">
            ${ring(profile.primaryRace.name || "Race", `${daysUntil(raceDate)}d`, percentBetween("2026-06-15", raceDate, date), "var(--blue)")}
        ...
        <article class="module-card stack">
          <div class="row-head">
            <div>
              ...
              <p class="subtle">${escapeHtml(profile.preferredGymTime)}${profile.gymLocation ? ` at ${escapeHtml(profile.gymLocation)}` : ""}</p>
            </div>
    ...
    return `
      <div class="set-row mobile-set ${completed ? "set-complete" : ""}">
        ...
        <div class="form-grid">
          ...
          <div class="field">
            <label>Reps</label>
            <input inputmode="numeric" type="number" step="1" min="0" value="${escapeAttr(set.reps)}" data-date="${date}" data-exercise-id="${exercise.id}" data-set-id="${set.id}" data-set-field="reps">
          </div>
        </div>
        <div class="field">
          <label>Notes</label>
  ...
  function renderFoodRow(date, entry) {
    return `
      <div class="food-row">
        <span><strong>${escapeHtml(entry.name)}</strong><br><span class="subtle">${formatNumber(entry.calories)} kcal - P ${formatNumber(entry.protein)} / C ${formatNumber(entry.carbs)} / F ${formatNumber(entry.fat)} / fibre ${formatNumber(entry.fibre)} - ${escapeHtml(entry.servingDescription || "1 serving")}${nutritionConfidenceLabel(entry)}</span></span>
  ...
  function nutritionConfidenceLabel(item) {
    const source = item.sourceType ? ` - ${item.sourceType.replace(/-/g, " ")}` : "";
    const confidence = item.confidence ? ` (${item.confidence})` : "";
    return item.estimated ? ` - Estimated nutrition${source}${confidence}` : `${source}${confidence}`;
  }
  ...
  function profileInput(name, label, type, value, step = "") {
    const stepAttr = step ? ` step="${escapeAttr(step)}"` : "";
    const inputMode = type === "number" ? ` inputmode="decimal"` : "";
    return `<div class="field"><label>${label}</label><input data-profile-input="${name}" type="${type}"${inputMode}${stepAttr} value="${escapeAttr(value)}"></div>`;
  }

  function trainingInput(name, label, type, value) {
    return `<div class="field"><label>${label}</label><input data-training-input="${name}" type="${type}" value="${escapeAttr(value)}"></div>`;
  }
  ...
  function renderFriendsProgressCard() {
    return `
      <article class="module-card stack">
        ...
        <div class="day-list">
          ${state.friends.items.length ? state.friends.items.map((friend) => renderFriendProgressRow(friend)).join("") : `<div class="empty">Add mates from More to see their weekly progress here.</div>`}
        </div>
      </article>
    `;
  }
    ...
    return `
      <article class="module-card stack">
        <div class="row-head">
          <div>
            <h3>Profile</h3>
            <p class="subtle">These settings drive dashboard times, body metric defaults and race countdowns.</p>
          </div>
          <span class="badge">${escapeHtml(profile.units === "imperial" ? "Imperial" : "Metric")}</span>
        </div>
        <div class="form-grid" id="profile-form">
          ${profileInput("displayName", "Display name", "text", profile.displayName)}
          ${profileInput("bodyweightKg", "Bodyweight kg", "number", profile.bodyweightKg, "0.1")}
          ${profileInput("heightCm", "Height cm", "number", profile.heightCm, "0.1")}
          ${profileInput("timezone", "Timezone", "text", profile.timezone)}
          <div class="field">
            <label>Units</label>
            <select data-profile-input="units">
              <option value="metric" ${profile.units === "metric" ? "selected" : ""}>Metric</option>
              <option value="imperial" ${profile.units === "imperial" ? "selected" : ""}>Imperial</option>
            </select>
          </div>
          ${profileInput("gymLocation", "Gym location", "text", profile.gymLocation)}
          ${profileInput("preferredRunTime", "Preferred run time", "time", profile.preferredRunTime)}
          ${profileInput("preferredGymTime", "Preferred gym time", "text", profile.preferredGymTime)}
          ${profileInput("raceName", "Race name", "text", profile.primaryRace.name)}
          ${profileInput("raceDate", "Race date", "date", profile.primaryRace.date)}
          ${profileInput("raceDistanceKm", "Race distance km", "number", profile.primaryRace.distanceKm, "0.1")}
        </div>
  ...
  function renderTrainingProgramSettings() {
    const training = state.training || migrateTrainingState({}, new Date().toISOString(), state.settings.deviceId);
    const templates = training.templates || {};
    const runningTemplates = Object.values(templates).filter((template) => template.type === "running");
    const gymTemplates = Object.values(templates).filter((template) => template.type === "gym");
    const running = training.assignedPrograms?.running || {};
    const gym = training.assignedPrograms?.gym || {};
    return `
      <article class="module-card stack">
        <div class="row-head">
          <div>
            <h3>Training Programs</h3>
            <p class="subtle">Template assignments are stored separately from completed sessions, so logs stay attached during future program changes.</p>
          </div>
          <span class="badge">Templates</span>
        </div>
        <div class="form-grid" id="training-form">
          <div class="field">
            <label>Running template</label>
            <select data-training-input="runningTemplateId">
              ${runningTemplates.map((template) => `<option value="${template.id}" ${template.id === running.templateId ? "selected" : ""}>${escapeHtml(template.name)}</option>`).join("")}
            </select>
          </div>
          ${trainingInput("runningStartDate", "Running start date", "date", running.startDate || PLAN_WEEKS[0].start)}
          <div class="field">
            <label>Gym template</label>
            <select data-training-input="gymTemplateId">
              ${gymTemplates.map((template) => `<option value="${template.id}" ${template.id === gym.templateId ? "selected" : ""}>${escapeHtml(template.name)}</option>`).join("")}
            </select>
          </div>
          ${trainingInput("gymStartDate", "Gym start date", "date", gym.startDate || PLAN_WEEKS[0].start)}
        </div>
        <button class="secondary" data-action="save-training-programs">Save Programs</button>
      </article>
    `;
  }
        ...
        ` : `
          <div class="form-grid" id="auth-form">
            <div class="field full">
              ...
              <input id="account-display-name" type="text" autocomplete="name" value="${escapeAttr(profile.displayName || state.profiles.list[state.settings.activeProfile]?.name || "Hybrid athlete")}" placeholder="Hybrid athlete">
            </div>
          </div>
          <div class="actions">
  ...
  function renderFriendsSettings() {
    ...
    const pendingCount = state.friends.requests?.length || 0;
    return `
      <article class="module-card stack">
        <div class="row-head">
          <div>
            <h3>Training Circle</h3>
            <p class="subtle">${state.auth.user ? "Friend codes send requests. Progress is shared only after both mates are connected." : "Create a cloud backup first, then add mates with friend codes."}</p>
          </div>
          <span class="badge">${state.friends.items.length} mates${pendingCount ? ` - ${pendingCount} pending` : ""}</span>
        </div>
        ...
        <div class="actions">
          <button class="primary" data-action="add-friend" ${state.auth.user ? "" : "disabled"}>Send Mate Request</button>
          <button class="secondary" data-action="refresh-friends" ${state.auth.user ? "" : "disabled"}>Refresh Mates</button>
        ...
        ${pendingCount ? `
          <div class="stack">
            <h4>Requests</h4>
            <div class="day-list">
              ${state.friends.requests.map(renderFriendRequestRow).join("")}
            </div>
          </div>
        ` : ""}
        <div class="day-list">
          ${state.friends.items.length ? state.friends.items.map((friend) => renderFriendProgressRow(friend, true)).join("") : `<div class="empty">No mates added yet.</div>`}
        </div>
      </article>
    `;
  }

  function renderFriendRequestRow(friend) {
    const friendCode = escapeAttr(friend.friendCode || "");
    return `
      <div class="food-row">
        <span>
          <strong>${escapeHtml(friend.displayName || "Mate")}</strong><br>
          <span class="subtle">Wants to connect training progress${friend.requestedAt ? ` - ${formatLongDate(friend.requestedAt.slice(0, 10))}` : ""}</span>
        </span>
        <span class="actions">
          <button class="primary" data-action="accept-friend" data-friend-code="${friendCode}">Accept</button>
          <button class="ghost" data-action="reject-friend" data-friend-code="${friendCode}">Reject</button>
        </span>
      </div>
    `;
  }

  function renderFriendProgressRow(friend, showControls = false) {
    ...
    return `
      <div class="food-row">
        ...
        ${showControls ? `<span class="actions">
          <span class="badge">${progress.latestBodyweight ? `${formatNumber(progress.latestBodyweight)} kg` : "Progress"}</span>
          <button class="ghost" data-action="remove-friend" data-friend-code="${friendCode}">Remove</button>
          <button class="ghost" data-action="block-friend" data-friend-code="${friendCode}">Block</button>
        </span>` : `<span class="badge">${progress.latestBodyweight ? `${formatNumber(progress.latestBodyweight)} kg` : "Progress"}</span>`}
      </div>
    `;
  }
  ...
  function completeNextSet(date, exerciseId) {
    ...
    if (next) {
      if (!confirmUnusualSet(next, exercise.name)) return false;
      next.completed = true;
  ...
  function completeSet(date, exerciseId, setId) {
    ...
    if (set) {
      if (!confirmUnusualSet(set, exercise?.name || "")) return false;
      set.completed = true;
      set.completedAt = new Date().toISOString();
      return true;
    }
  ...
  function confirmUnusualSet(set, exerciseName = "") {
    const unusual = setLooksUnusual(set, exerciseName);
    if (unusual.highReps) return confirm(`${set.reps} reps for ${exerciseName || "this set"} looks unusual. Complete it anyway?`);
    if (unusual.highWeight) return confirm(`${set.weight} kg for ${exerciseName || "this set"} looks unusual. Complete it anyway?`);
    return true;
  }
  ...
  function addFoodFromForm() {
    ...
    state.nutrition.entries[ui.foodDate].push(normalizeFoodRecord({ ...entry, ...newRecordMeta(id("food")), source: "manual" }, "user-entered"));
  ...
  function savePresetFromForm() {
    ...
    state.nutrition.presets.push(normalizeFoodRecord({ ...preset, id: id("preset"), isPreset: true }, "user-entered"));
  ...
  function addPresetToDay(presetId, date) {
    ...
    state.nutrition.entries[date].push(normalizeFoodRecord({ ...preset, ...newRecordMeta(id("food")), meal: mealForTime(), source: "preset" }, preset.sourceType || "package-label"));
  ...
  function logMealPrep(prepId) {
    ...
    if (!state.nutrition.entries[ui.foodDate]) state.nutrition.entries[ui.foodDate] = [];
    state.nutrition.entries[ui.foodDate].push(normalizeFoodRecord({
  ...
  function saveProfileSettings() {
    const next = { ...currentProfile(), primaryRace: { ...currentProfile().primaryRace } };
    document.querySelectorAll("[data-profile-input]").forEach((input) => {
      const key = input.dataset.profileInput;
      if (key === "raceName") next.primaryRace.name = cleanProfileText(input.value, 80);
      else if (key === "raceDate") next.primaryRace.date = isISODate(input.value) ? input.value : next.primaryRace.date;
      else if (key === "raceDistanceKm") next.primaryRace.distanceKm = optionalNumber(input.value) || next.primaryRace.distanceKm;
      else if (key === "bodyweightKg" || key === "heightCm") next[key] = optionalNumber(input.value);
      else if (key === "preferredRunTime") next[key] = normalizeProfileTime(input.value);
      else if (key === "units") next[key] = input.value === "imperial" ? "imperial" : "metric";
      else if (key === "displayName") next[key] = cleanProfileText(input.value, 60) || DEFAULT_PROFILE.displayName;
      else next[key] = cleanProfileText(input.value, 100);
    });
    state.profile = migrateProfile(next, SCHEMA_VERSION, true);
    state.settings.timezone = state.profile.timezone;
    state.settings.units = state.profile.units;
    if (state.profiles?.list?.[state.settings.activeProfile]) {
      state.profiles.list[state.settings.activeProfile].name = state.profile.displayName || state.profiles.list[state.settings.activeProfile].name;
    }
  }

  function saveTrainingPrograms() {
    if (!state.training) state.training = migrateTrainingState({}, new Date().toISOString(), state.settings.deviceId);
    const running = state.training.assignedPrograms.running;
    const gym = state.training.assignedPrograms.gym;
    document.querySelectorAll("[data-training-input]").forEach((input) => {
      const key = input.dataset.trainingInput;
      if (key === "runningTemplateId" && state.training.templates[input.value]) running.templateId = input.value;
      if (key === "gymTemplateId" && state.training.templates[input.value]) gym.templateId = input.value;
      if (key === "runningStartDate" && isISODate(input.value)) running.startDate = input.value;
      if (key === "gymStartDate" && isISODate(input.value)) gym.startDate = input.value;
    });
  ...
  function currentProfile() {
    if (!state.profile) state.profile = migrateProfile({}, SCHEMA_VERSION, false);
  ...
  function copyPreviousMeal() {
    ...
    entries.forEach((entry) => {
      state.nutrition.entries[ui.foodDate].push(normalizeFoodRecord({ ...entry, ...newRecordMeta(id("food")), date: ui.foodDate }, entry.sourceType || "user-entered"));
      updateFoodUsage(entry.name);
    });
  }
  ...
  async function lookupBarcode(code) {
    ...
    try {
      ...
      if (ok) {
        if (!state.nutrition.entries[ui.foodDate]) state.nutrition.entries[ui.foodDate] = [];
        state.nutrition.entries[ui.foodDate].push(normalizeFoodRecord({ ...product, ...newRecordMeta(id("food")), meal: mealForTime(), source: "barcode" }, "barcode"));
        state.nutrition.presets = cleanFoodPresets([...state.nutrition.presets, { ...product, id: id("preset"), isPreset: true }]);
        state.nutrition.foods = cleanFoodPresets([...(state.nutrition.foods || []), ...state.nutrition.presets]);
  ...
  async function fetchOpenFoodFacts(code) {
    ...
    return {
      ...
      sodium: toNumber(nutriments.sodium_serving || nutriments.sodium_100g),
      water: 0,
      caffeine: 0,
      estimated: false,
      source: "Open Food Facts",
      sourceType: "barcode",
      sourceReference: `Open Food Facts:${code}`,
      confidence: data.product.last_modified_t ? "high" : "medium",
      servingDescription: data.product.serving_size || "1 serving",
      servingWeightGrams: toNumber(data.product.serving_quantity || ""),
      verifiedAt: data.product.last_modified_t ? new Date(data.product.last_modified_t * 1000).toISOString() : "",
  ...
  async function createCloudAccount() {
    ...
    const displayName = (input?.value || currentProfile().displayName || state.profiles.list[state.settings.activeProfile]?.name || "Hybrid athlete").trim();
  ...
  async function addFriend() {
    ...
    try {
      ...
      await refreshFriends({ silent: true });
      alert(`Mate request sent to ${result.friend?.displayName || "your mate"}. They need to accept before progress is shared.`);
    } catch (error) {
  ...
  async function refreshFriends(options = {}) {
    ...
    try {
      const [progressResponse, requestResponse] = await Promise.all([
        fetch(`${API_BASE}/friends/progress`, { credentials: "include" }),
      ...
      if (!progressResponse.ok) throw new Error(progressResult.error || `Refresh failed (${progressResponse.status})`);
      if (!requestResponse.ok) throw new Error(requestResult.error || `Requests failed (${requestResponse.status})`);
      state.friends.items = progressResult.friends || [];
      state.friends.requests = requestResult.requests || [];
      state.friends.lastFetchedAt = progressResult.fetchedAt || new Date().toISOString();
      state.friends.pendingLastFetchedAt = requestResult.fetchedAt || new Date().toISOString();
  ...
  async function updateFriendRelationship(friendCode, action) {
    if (!state.auth.user || !friendCode) return;
    if (action === "block" && !confirm("Block this mate? They will not be able to send another request from this code.")) return;
    try {
      const response = await fetch(`${API_BASE}/friends/${action}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ friendCode })
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || `Mate update failed (${response.status})`);
      await refreshFriends({ silent: true });
    } catch (error) {
      state.friends.lastError = error.message || "Could not update mate.";
      alert(state.friends.lastError);
    }
    saveState({ skipSync: true });
    render();
  }
